...
Decoding lives in `src/tds/tokens.c:parse_cell` and the `tds_result_get_*` family in `src/tds/result.c`. We support enough types to drive `sys.databases`, `sys.servers`, `sys.server_principals`, `sys.configurations`, and the introspection bundle the `info` action runs. Unsupported types (`SQL_VARIANT`, `XML`, geography) decode to a `<unsupported>` placeholder.
...
Then read `src/bofs/mssql.c` to see how thin each action's shim is once the protocol layer exists.
